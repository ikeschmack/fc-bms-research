...
with PdfPages(pdf_file_path) as pdf_pages:
    ...
    for sub_job_id, job_group in df_flattened.groupby('sub_job_id'):
        ...
        if worker_90th_early_exit is not None:
            worker_90th_early_exit /= len(job_group['worker_name'].unique())
            ax.axhline(y=worker_90th_early_exit, color='purple', linestyle='--', label='Average 90th Percentile Bandwidth (Early Exit)')
        ...
        # Create a legend with the values of the 90th percentiles
        legend_labels = []
        if worker_90th_percentile is not None:
            legend_labels.append(f'Average 90th Percentile Bandwidth (All Logs): {worker_90th_percentile:.2f} Bps')
        if worker_90th_early_exit is not None:
            legend_labels.append(f'Average 90th Percentile Bandwidth (Early Exit): {worker_90th_early_exit:.2f} Bps')
        ax.legend(title='\n'.join(legend_labels), loc='upper right', fontsize='small')
