#using "cesginc.com" as the sample storage node to find out the useful angles to 
...
print(f"总共读取 {len(data)} 条job记录")

for idx, job in enumerate(data):
    node_id = None
    url = job.get('url', '')
    if url:
        # 简单抽取node_id，可以用hostname:port的形式
...
# 保存csv
csv_path = 'node_job_subjob_throughput_debug.csv'
